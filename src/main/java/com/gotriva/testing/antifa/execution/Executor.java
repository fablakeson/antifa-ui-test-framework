...
public class Executor {
        ...
        for (Command command : commands) {
            ...
            try {
                ...
                if (handler instanceof InteractableHandler) {
                    ...
                    handleInteractable(
                        (InteractableHandler<Interactable>)handler, 
                        interactable, command.getParameter());   
                    ...
                    throw new RuntimeException("Handler for command '" 
                            + command.getCommand() + "' not found.");
                }
                ...
                result.addStep(new ExecutionStep(
                    command, 
                    startTime, 
                    LocalDateTime.now(), 
                    ExecutionStep.Result.SUCCESS, 
                    snapshot));
    ...
    /**
     * Gets the interactable on context for given command.
    ...
    private Interactable getInteractableFor(Command command) {
        ...
        if (!pageObject.hasElement(command.getObject())) {
            ...
            String type = command.getType() != null 
                ? command.getType()
                : defaultTypes.get(command.getCommand());
            
            /** Creates the object on this page */
            pageObject.addElement(
                    command.getObject(), 
                    By.name(command.getObject()), 
                    type);
        }
